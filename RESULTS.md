...
As expected, we find that suboptimal index orderings perform much worse. Below is a comparison of Reordered, Naive and Multithreaded_c.

![Generated by compare_index_strategies in reproducible_results.py](./benchmarking/assets/basic_test.png)

Unsurprisingly, Reordered crushes Naive. Yet it also crushes Multithreaded_c - a multithreaded implementation which varies over k before j. This underlines the significance of cache misses to performance.

## Comparison of tiling strategies (single-threaded)

As expected, by tiling our matrices we gain a huge performance speed-up, particularly for larger matrices. Computing a row of the output matrix requires every element of the right matrix to be read. Through tiling, a smaller number of elements are read and used at a time, reducing cache misses.

We have two tiling implementations: Tiled_a which computes tiles sequentially, and Tiled_b which computes them recursively.

### Tile Size

We begin by tuning our parameter sizes on a square matrix:
...
Strikingly, we see that reordered keeps pace with (and even beats) Tiled_a and Tiled_b when we shrink the width and the ksize! With a bit of thought, this makes perfect sense. Tiling maintains performance because a row of the output requires the entirety of the right matrix. But if we shrink the width or height of the output, this argument becomes less salient. Yet the argument still applies if we change the height of the ouput (and left) matrices. And so in this case tiling consistently outperforms.
