...
Unsurprisingly, Reordered greatly outperforms Naive. Yet it also outperforms Multithreaded_c - a multithreaded implementation which loops over j before k. This underlines the significance of cache misses to performance.
...
I was also surprised by the lack of any real performance improvement of Tiled_b over Tiled_a. I expected Tiled_b to be faster because its recursive calls should be better suited to exploiting multiple cache layers. But we can see from the tables Tiled_a is consistently faster at different tile sizes, at least for 2048x2048 matrices.
...
This suggests an asymmetry - the tiling of the right matrix is more important for maintaining performance than the tiling of the left and output matrices.
...
* **Investigation idea:** *These tests are running on my linux mint gaming laptop. I also have a quieter headless arch server. Would performing the tests there yield more predictable results in terms of optimal thread numbers?*
...
Interestingly, we see that though recursive maintains a consistent lead in most cases, it starts to fall behind for smaller ksizes. After some thought, I still can't figure out why that would be the case. Neither implementation parallelizes over k, but I'm not sure how that would affect this.
